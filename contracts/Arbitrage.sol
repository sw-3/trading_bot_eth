...
contract Arbitrage is DyDxFlashLoan {
    ...
    IUniswapV2Router02 public immutable tRouter;

    address public owner;

    constructor(address _sRouter, address _uRouter, address _tRouter) {
        sRouter = IUniswapV2Router02(_sRouter); // Sushiswap
        uRouter = IUniswapV2Router02(_uRouter); // Uniswap
        tRouter = IUniswapV2Router02(_tRouter); // ShibaSwap
        owner = msg.sender;
    }

    function executeTrade(
        uint256 _firstExchange,
        uint256 _secondExchange,
        address _token0,
        address _token1,
    ...
    function callFunction(
        ...
        (
            uint256 firstExchange,
            uint256 secondExchange,
            address token0,
            address token1,
            uint256 flashAmount,
        ...
        } else if (firstExchange == 1) {
            _swapOnSushiswap(path, flashAmount, 0);
        
        } else {
            _swapOnShibaswap(path, flashAmount, 0);
        }
    ...
    function _swapOnShibaswap(
        address[] memory _path,
        uint256 _amountIn,
        uint256 _amountOut
    ) internal {
        require(
            IERC20(_path[0]).approve(address(tRouter), _amountIn),
            "ShibaSwap approval failed."
        );

        tRouter.swapExactTokensForTokens(
            _amountIn,
            _amountOut,
            _path,
            address(this),
